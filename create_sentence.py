...
class Sentence:
    ...
    def __init__(self, sentence_file, stretch_factor):
        text_file = open(sentence_file, "r") # open file, read mode
        ...
        for word_name in lines:               # iterate through words (text) as only one word per line
            ...
            audio_seg_word=AudioSegment.from_file("sound_files/"+word_name+".wav") 	# open as audiosegment to strip silence 
            word_freq, w=wavfile.read("sound_files/"+word_name+".wav") 		   	   	# do this to get sample rate
            
            word = wave.open("sound_files/"+word_name+".wav")						# do this to convert to floating point 1 to -1
...
def main(argv):
	...
	control_words=[]
   	...
   	for i in range(s.length):
  		...
  		# stretch_factor=old_l/new_l # eg stretch factor 0.5 -> 0.5 times original
   		stretch_factor=1#new_l/old_l
   		# s.words[i]=stretch_simple(s.words[i], stretch_factor)
   		s.words[i]=stretch(s.words[i], stretch_factor)
   		control_words.append(control(s.freq, s.words[i], new_l, coefs, num_gammatone_bands))
	...
	# fig3=plt.figure()					# plot the fft spectrogram
	# h=fig3.add_subplot(111)
	# # NFFT window size in samples
	# NFFT=11025
	# Pxx, freqs, bins, im=h.specgram(wav_sentence, NFFT=NFFT, Fs=s.freq, noverlap=int(NFFT/1.15),
	# 					cmap=None, xextent=(0,t_end), pad_to=44100, sides='default',
	# 					scale_by_freq=None, mode='psd', scale='dB')

	# # fig4=plt.figure()					# plot the fft spectrogram
	# # h=fig4.add_subplot(111)
	# # nperseg=2**8
	# # nfft=44100
	# # # f, t, Zxx = stft(wav_sentence, fs=s.freq, nperseg=nperseg, noverlap=int(nperseg/1.15), nfft=nfft)
	# # # h.pcolormesh(t, f, np.abs(Zxx))
	# # f, t, Zxx=spectrogram(wav_sentence, fs=s.freq, window=('tukey', 0.25), nperseg=nperseg, noverlap=int(nperseg/1.15),
	# # 			nfft=nfft, detrend='constant', return_onesided=True, scaling='spectrum', axis=-1, mode='psd')
	# # h.pcolormesh(t, f, Zxx)
	# # h.set_ylim([f[1], f[-1]])
	# # h.set_title('STFT Magnitude')
	# # h.set_ylabel('Frequency [Hz]')
	# # h.set_xlabel('Time [sec]')
	# # h.set_yscale('log')
	...
	# recon_specgram[recon_specgram < -1.65] = -2
	# print 'shape specgram', specgram.shape
	# print 'shape recon_specgram', recon_specgram.shape
