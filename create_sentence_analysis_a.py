...
class Sentence:
    ...
    def __init__(self, sentence_file, deviation_factor):
...
def gammatone_envelopes(samp_freq,sound, gammatone_coeffs, num_gammatone_bands):
...
def main(argv):
	...
	try:                                                   
		opts, args = getopt.getopt(argv,"hi:d:",["snum=","dev="])   
	except getopt.GetoptError:                            
		print 'create_sentence_analysis_a.py -i <sentence num> -d <deviation factor>'       
		sys.exit(2)                                        
	for opt, arg in opts:                                 
		if opt == '-h':
			print 'create_sentence_analysis_a.py -i <sentence num> -d <deviation factor>'
			...
			deviation_factor = float(arg)                      

	sentence_num = str(sentence_file)
	s = Sentence(sentence_file, deviation_factor)               # create a Sentence object s

	control_words = []

   	for i in range(s.length):
		old_l = s.t_lengths[i]
		new_l = ave_l + s.deviation_factor*(old_l-ave_l)
	...
	envelope = gammatone_envelopes(s.freq, wav_sentence, gammatone_coeffs=coefs, num_gammatone_bands=num_gammatone_bands)
	############################	
